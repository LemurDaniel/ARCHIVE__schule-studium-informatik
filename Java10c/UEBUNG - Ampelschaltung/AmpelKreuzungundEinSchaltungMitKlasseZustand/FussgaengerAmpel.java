public class FussgaengerAmpel
{
    ...
    {        
        switch (position)
        {
            ...
            case "Osten":
            Gehaeuse=new KASTEN(links,oben,
                            4*breite+3*rand,2*breite+2*rand); //Kasten - liegend
            Signale[0]= new VOLLKREIS(links+breite+rand,oben+breite+rand,breite,4); // Lichter alle mit gleichem Oben-Wert
            Signale[1]= new VOLLKREIS(links+3*breite+2*rand,oben+breite+rand,breite,2);
            break;
            
            case "Sueden":
            Gehaeuse=new KASTEN(links,oben,
                            2*breite+2*rand,4*breite+3*rand); //Kasten - stehend; Lichter alle mit gleichem Links-Wert 
            Signale[0]= new VOLLKREIS(links+breite+rand,oben+breite+rand,breite,4); // Lichter: rot unten
            Signale[1]= new VOLLKREIS(links+breite+rand,oben+3*breite+2*rand,breite,2);
            break;
