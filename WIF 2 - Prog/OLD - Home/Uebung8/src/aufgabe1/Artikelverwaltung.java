...
public class Artikelverwaltung extends JFrame {
			...
			btnSpeichern.addActionListener( al -> {
				try {
					...
					if(textFieldBez.getText().equals("TestList")) {
						new Artikel.TestArtikelListe().forEach(artk->((DefaultListModel<Artikel>)list.getModel()).addElement(artk));
						return;
					}
					
					((DefaultListModel<Artikel>)list.getModel()).addElement(new Artikel(textFieldBez.getText(), textFieldPreis.getText(), textFieldMenge.getText()));
	...
	private JButton getBtnLoeschen() {
		if (btnLoeschen == null) {
			...
			btnLoeschen.addActionListener( al->{
				lblStatus.setText(null);
				((DefaultListModel<Artikel>)list.getModel()).remove(list.getSelectedIndex()); 
				list.updateUI();
			});
		}
		return btnLoeschen;
	...
	private JList<Artikel> getList() {
		if (list == null) {
			list = new JList<>(new DefaultListModel<Artikel>());
