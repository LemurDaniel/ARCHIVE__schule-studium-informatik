...
public class ST_GUI extends JFrame {
	...
	public ST_GUI() {
		...
		getContentPane().add(getBtnLschen());
		...
		if (btnErfassen == null) {
			...
			btnErfassen.addActionListener( e -> {
				...
				try {					
					...
					String name = Student.checkName(tfName.getText());
				...
				if(cBstudents.getSelectedIndex() == cBstudents.getModel().getSize()-1) {
					...
					cBstudents.setSelectedIndex(cBstudents.getModel().getSize()-1);
				}else {
	...
	private JButton getBtnLschen() {
		if (btnLschen == null) {
			...
			btnLschen.setFont(new Font("Dialog", Font.BOLD, 13));
			btnLschen.setBounds(323, 181, 133, 23);
			btnLschen.setVisible(false);
			btnLschen.addActionListener(l->{
				if( JOptionPane.showConfirmDialog(ST_GUI.this, "Wollen sie diesen Studenten wirklich löschen", "Warnung", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) != JOptionPane.YES_OPTION)
					return;
				students.remove(cBstudents.getSelectedIndex());
				dcbmStudents.removeElementAt(cBstudents.getSelectedIndex());
			});
		}
		return btnLschen;
	}
