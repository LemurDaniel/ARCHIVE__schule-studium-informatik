...
//		
	...
	private void setStandarOP() {
		...
		map.put("endifloop", () -> {
			...
			cr.accept(String.format("  Start ENDIFLOOP(%s)(%s)", arg, arg2));
			stack.push(arg);
			while(stack.peek().getClass().equals(String.class)) map.get("=").operate();
			if(!stack.peek().getClass().equals(Boolean.class)) map.get("boolean").operate();
			while(((Boolean)stack.pop()).equals(Boolean.TRUE)) {
				if(stack.peek().toString().equals(arg2)) stack.pop();
				calcExp(arg2);
				calcExp(arg);
			}
			cr.accept(String.format("  End ENDIFLOOP(%s)(%s)", arg, arg2));
		});
		...
		map.put("unmap", () -> map.remove(stack.pop().toString()) );
