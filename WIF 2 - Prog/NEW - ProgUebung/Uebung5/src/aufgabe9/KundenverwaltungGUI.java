...
public class KundenverwaltungGUI extends SicherBeenden implements ActionListener {
	...
	private Button getBtn_speichern() {
		if (btn_speichern == null) {
			...
			btn_speichern.addActionListener(this::actionPerformed);
		}
	...
	private Button getBtn_loeschen() {
		if (btn_loeschen == null) {
			...
			btn_loeschen.addActionListener(this::actionPerformed);
		}
		return btn_loeschen;
	}
	...
	private TextField getTF_kundenname() {
		if (tF_kundenname == null) {
			...
			tF_kundenname.addKeyListener(new KeyAdapter() {
				@Override
				public void keyReleased(KeyEvent e) {
					//Gleiches mit anderen Feldern um Focus zu traversieren
					if(e.getKeyCode() == KeyEvent.VK_ENTER) tF_Firmenadresse.requestFocus();
				}
			});
		}
	...
	public void storeKunde() {
		if( tF_kundenname.getText().length() >2 && tF_Firmenadresse.getText().length() >2) {
			...
			kunden[choice_Kunde.getSelectedIndex()] = new Kunde(tF_kundenname.getText(), tF_Firmenadresse.getText());
