...
public class ListensichtCtrl {
	...
	private void zuruecksetzen() throws Exception{
		if(tab_film.isSelected()) {
			if(angezeigteListe==null)	throw new Exception("Es wurde keine Liste ausgewaehlt");
			angezeigteListe.reset();
		}else 
			lvw.reset();		
		FensterManager.logErreignis("\nAlle ungespeichertena enderungen wurden zurueckgesetzt");
	}
	...
	private void updateFilm() throws Exception{
		if(angezeigteListe==null || table_film.getSelectionModel().getSelectedItem()==null) throw new Exception("Es wurde kein Film ausgewaehlt");
    ...
    void initialize() {
    	...
    	tListe_size.setCellValueFactory(data->data.getValue().getGroesseProperty());
    	...
    	tab_listen.selectedProperty().addListener((ob,ov,nv)->{
    		if(nv=true)	lvw.getObList().forEach(li->li.aktualisiereGroesse());
    	});
