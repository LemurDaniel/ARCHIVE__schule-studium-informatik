...
public class Liste extends Stapelverarbeitung<Film> implements Backup, EingabePruefung, Id {
	...
	private int id;
	private ReadOnlyStringWrapper name, groesse;
	...
	public Liste(int id, String name) {
		...
		groesse = new ReadOnlyStringWrapper("0");
	}
	...
	public ReadOnlyStringProperty getGroesseProperty() {
		return groesse.getReadOnlyProperty();
	}
	...
	protected void onAdd(Film film, Connection con) throws Exception {
		if(film.getId()==-1)	throw new Exception("Der Film '"+film.getTitel()+"' konnte nicht zur liste hinzugef�gt werden");
	...
	protected void onDeleteSucess(Film film, Connection con) {
		...
		aktualisiereGroesse();
		FensterManager.logErreignis(String.format("'%s' wurde erfolgreich aus Liste '%s' gel�scht", film.getTitel(), name.get()));
	}
